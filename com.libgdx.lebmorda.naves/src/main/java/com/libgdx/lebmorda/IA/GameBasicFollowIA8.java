...
public class GameBasicFollowIA8 extends ScreenAdapter {
	...
	public GameBasicFollowIA8(BasicGame game) {
		this.game = game;
	...
	private void configWorld() {
		...
		createSprinteBody(nave, "nave", 0, 0, 0);
	}
	 ...
	 */
	private Body createSprinteBody(SpriteActor actor, String name, float x, float y, float rotation) {
		PolygonShape square= new PolygonShape();
		square.setAsBox(50, 100);
		createShapeBodyFactory(square).setTransform(0,0,0);
		createSpriteBodyFactory(name, actor);
	...
	private Body createSpriteBodyFactory(String name, SpriteActor actor) {
		// Creamos el bodyDef
	...
	private Body createShapeBodyFactory(Shape shape) {
		// Creamos el bodyDef
		BodyDef bodyDef = createBodyDef();
		Body body = world.createBody(bodyDef);
		if (shape.getType() == Shape.Type.Edge) {
			shape = new EdgeShape();
		} else if (shape.getType() == Shape.Type.Circle) {
			shape = new CircleShape();
		} else if (shape.getType() == Shape.Type.Chain) {
	...
	public void render(float delta) {
		...
		gamecam.position.x = naveBody.getPosition().x;
		gamecam.position.y = naveBody.getPosition().y;
	...
	private void inputListener(float delta) {
		...
		if (vController.isMoverArriba()) {
			naveBody.applyForceToCenter(0f, 1000000000, true);
		}
		...
		if (vController.isGirarDerecha()) {
			naveBody.applyForceToCenter(1000000f, 0, true);
