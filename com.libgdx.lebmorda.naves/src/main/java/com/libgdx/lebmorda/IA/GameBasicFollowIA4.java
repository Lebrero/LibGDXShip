...
public class GameBasicFollowIA4 extends ScreenAdapter {

	private static final int ZERO_POSITION_Y = 0;
	private static final int ZERO_POSITION_X = 0;
	private static final int VELOCIDAD_GIRO = 90;
	private static final int VELOCIDAD_AVANCE = 10;
	...
	public GameBasicFollowIA4(BasicGame game) {
		this.game = game;
	...
	private void inputListener(float delta) {
		...
		if (vController.isGirarDerecha()) {
			nave.centerOriginXY().rotateActor(-VELOCIDAD_GIRO);
			...
			vController.setGirarDerecha(false);
		}
		...
		if (vController.isGirarIzquierda()) {
			...
			vController.setGirarIzquierda(false);

		}

		nave.addAction(Actions.moveBy(posX, posY));
