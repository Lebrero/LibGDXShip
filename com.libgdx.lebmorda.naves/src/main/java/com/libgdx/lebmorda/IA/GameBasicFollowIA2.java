...
public class GameBasicFollowIA2 extends ScreenAdapter {

	private static final int PIXELS_TO_2D = 90;
	private static final int VELOCIDAD_GIRO = 10;
	...
	public GameBasicFollowIA2(BasicGame game) {
		this.game = game;

		gamecam = new OrthographicCamera();
		gameport = new FitViewport(6,9, gamecam);
	...
	public void show() {
		init();
		stage.addActor(centro.setDimensionActor(250/PIXELS_TO_2D, 250/PIXELS_TO_2D).centerOrigin());
		stage.addActor(nave.setDimensionActor(nave.getWidth()/PIXELS_TO_2D, nave.getHeight()/PIXELS_TO_2D).centerOrigin());
	}
	...
	private Body createBodyFactory(String name) {
		...
		square.setAsBox(naveWidth, naveHeight);
	...
	private void inputListener(float delta) {
		...
		if (vController.isMoverArriba()) {
			...
			// forever in circles, clockwise, with rotating of the actor...
			nave.centerOriginXY().addAction(Actions.forever(ActionMoveCircular.actionCircle(1, 1, 1, 1, true, 360)));

			// wide ellipse, anticlockwise, without rotating the actor
		//	actor.addAction(Actions.forever(ActionMoveCircular.actionEllipse(x, y, r, r/2, -1, false, 360)));
