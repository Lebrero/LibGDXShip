...
public class GameBasicFollowIA5 extends ScreenAdapter {
	...
	public GameBasicFollowIA5(BasicGame game) {
		this.game = game;
	...
	private void inputListener(float delta) {
		...
		if (vController.isGirarDerecha()) {
			...
			if ((avanceX >= 0 && avanceX <= 5) && (avanceY >= 0 && avanceY <= 5)) {
				avanceX = avanceX + 5 * delta;
				avanceY = avanceY - 5 * delta;
		...
		nave.addAction(Actions.moveBy(avanceX, avanceY));
